...
def over18(board):
    res = rs.get('https://www.ptt.cc/bbs/' +
                 board + '/index.html', verify=False)
    ...
    for tag in soup.select('div.push'):
        try:
            ...
            push_ipdatetime = tag.find(
                "span", {'class': 'push-ipdatetime'}).text
...
def store(data):
    with open(fileName, 'a', encoding='utf8') as f:
        f.write(data.encode(sys.stdin.encoding,
...
if __name__ == "__main__":
    ...
    for index in range(ALLpage, ALLpage - int(ParsingPage), -1):
        page_url = 'https://www.ptt.cc/bbs/' + \
            PttName + '/index' + str(index) + '.html'
